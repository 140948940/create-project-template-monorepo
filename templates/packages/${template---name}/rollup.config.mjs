...
const outputConfigs = {
  ...
  browser: {
    file: 'dist/${template---name}.esm-browser.js',
    format: `es`,
  },
}

const stubs = {
  'dist/${template---name}.cjs': '${template---name}.cjs.js',
  'dist/${template---name}.mjs': '${template---name}.esm-bundler.js',
  'dist/${template---name}.prod.cjs': '${template---name}.cjs.prod.js',
}

const packageBuilds = Object.keys(outputConfigs)
// in ${template---name} there are not that many
...
console.log('packageBuilds',packageBuilds)
